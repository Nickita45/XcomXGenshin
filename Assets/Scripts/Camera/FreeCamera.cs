...
public class FreeCamera : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        _speed = ConfigurationManager.GlobalDataJson.cameraSpeed;
        _speedRotation = ConfigurationManager.GlobalDataJson.cameraSpeedRotation;
        _speedZoom = ConfigurationManager.GlobalDataJson.cameraSpeedZoom;
        _zoomMin = ConfigurationManager.GlobalDataJson.cameraZoomMin;
        _zoomMax = ConfigurationManager.GlobalDataJson.cameraZoomMax;
    ...
    {
        if (Manager.Map == null ||
            (!Manager.HasPermission(Permissions.CameraMovements) &&
            !Manager.HasPermission(Permissions.Waiting)))
            return;
    ...
    private void HandleMovementInput()
    {
        ...
        if (Manager.Map != null && Manager.Map.width != 0) ApplyBounds(ref newPosition);
    ...
    {
        Vector3 center = Manager.Instance.transform.position;
        (float width, float height)
            = (Manager.Map.width, Manager.Map.width);
